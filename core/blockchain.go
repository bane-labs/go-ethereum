...
func (bc *BlockChain) InsertHeadersBeforeCutoff(headers []*types.Header) (int, error) {
	...
	if first == 1 && frozen == 0 {
		_, err := rawdb.WriteAncientBlocks(bc.db, []*types.Block{bc.genesisBlock}, []rlp.RawValue{rlp.EmptyList}, bc.genesisBlock.Difficulty())
		if err != nil {
			log.Error("Error writing genesis to ancients", "err", err)
			return 0, err
		}
...
func (bc *BlockChain) ProcessState(block *types.Block, statedb *state.StateDB) (*state.StateDB, *ProcessResult, error) {
	...
	res, err := bc.processor.Process(context.Background(), block, statedb, nil, bc.cfg.VmConfig, nil)
	if err != nil {
...
// VerifyBlock validates Block body built by dBFT engine is allowed by Geth rules, and
// returns necessary results after processing. Here we compute the BAL instead of using it,
// since it can be computed locally and is not included in dBFT proposals.
func (bc *BlockChain) VerifyBlock(block *types.Block) (*state.StateDB, types.Receipts, uint64, *bal.ConstructionBlockAccessList, error) {
	...
	if block.BaseFee().Cmp(baseFee) != 0 {
		return nil, nil, 0, nil, fmt.Errorf("failed to verify policy baseFee, expected: %v, current: %v", baseFee, block.BaseFee())
	}
	// compute the state and block access list
	...
	if err := bc.validator.ValidateState(block, statedb, res, false); err != nil {
		return nil, nil, 0, nil, fmt.Errorf("failed to verify state: %w", err)
	}
	return statedb, res.Receipts, res.GasUsed, res.Bal, nil
}
