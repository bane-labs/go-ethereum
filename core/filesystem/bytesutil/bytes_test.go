...
import (
	...
	"github.com/ethereum/go-ethereum/core/filesystem/bytesutil"
...
func TestReverseByteOrder(t *testing.T) {
	...
	require.Equal(t, bytes.Equal(input, []byte{0, 1, 2, 3, 4, 5}), true)
	require.Equal(t, bytes.Equal(expectedResult, output), true)
}
...
func TestSafeCopy2d32Bytes(t *testing.T) {
	...
	require.Equal(t, false, &input == &output, "No copy was made")
...
func TestSafeCopy2dHexUtilBytes(t *testing.T) {
	...
	require.Equal(t, output, [][]byte{{'a'}, {'b'}})
}
...
func TestCastToString(t *testing.T) {
	...
	bSlice[0] = 'd'
	require.NotEqual(t, originalString, bString)
	require.Equal(t, "dbc", bString)
}
...
func TestSafeCopyBytes_Copy(t *testing.T) {
	...
	require.NotEqual(t, fmt.Sprintf("%p", slice), fmt.Sprintf("%p", copiedSlice))
	require.Equal(t, slice[0], copiedSlice[0])
	slice[1] = 'B'

	require.NotEqual(t, slice[1], copiedSlice[1])
}
