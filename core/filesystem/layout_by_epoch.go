...
type periodicEpochLayout struct {
	...
	pruner *blobPruner
	retain primitives.Epoch
}

var _ fsLayout = &periodicEpochLayout{}

func newPeriodicEpochLayout(fs afero.Fs, cache *blobStorageSummaryCache, pruner *blobPruner, retain primitives.Epoch) fsLayout {
	l := &periodicEpochLayout{fs: fs, cache: cache, pruner: pruner, retain: retain}
	return l
}
...
func (l *periodicEpochLayout) iterateIdents(before primitives.Epoch) (*identIterator, error) {
	...
	return &identIterator{
		...
		layers: []layoutLayer{
			{populateIdent: populateNoop, filter: isBeforePeriod(before, l.retain)},
...
func (l *periodicEpochLayout) periodDir(epoch primitives.Epoch) string {
	return filepath.Join(PeriodicEpochBaseDir, fmt.Sprintf("%d", periodForEpoch(epoch, l.retain)))
}
...
func periodForEpoch(epoch, retain primitives.Epoch) primitives.Epoch {
	return epoch / retain
}

// Funcs below this line are iteration support methods that are specific to the epoch layout.

func isBeforePeriod(before, retain primitives.Epoch) func(string) bool {
	if before == 0 {
		return filterNoop
	}
	beforePeriod := periodForEpoch(before, retain)
