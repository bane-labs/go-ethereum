...
func directoryPermissions() os.FileMode {
	return file.DefaultIoConfig().ReadWriteExecutePermissions
}
...
// WithChainConfig allows the user to specify a custom chain configuration.
func WithChainConfig(cfg *params.ChainConfig) BlobStorageOption {
	return func(b *BlobStorage) error {
		b.cfg = cfg
		return nil
	}
...
func NewBlobStorage(opts ...BlobStorageOption) (*BlobStorage, error) {
	...
	layout, err := newLayout(b.layoutName, b.fs, b.cache, pruner, b.retentionEpochs)
	if err != nil {
		return nil, err
	}
	b.layout = layout
...
func (bs *BlobStorage) migrateLayouts() error {
	for _, name := range LayoutNames {
		...
		from, err := newLayout(name, bs.fs, bs.cache, nil, bs.retentionEpochs)
		if err != nil {
			return err
		}
...
func (bs *BlobStorage) writePart(sidecar types.VerifiedROBlob) (ppath string, err error) {
	ident := identForSidecar(sidecar)
	sidecarData, err := rlp.EncodeToBytes(sidecar.ROBlob.BlobSidecar)
	if err != nil {
...
func (bs *BlobStorage) Save(sidecar types.VerifiedROBlob) error {
	startTime := time.Now()
	...
	if exists {
		log.WithFields(logrus.Fields{
			"blockNumber": sidecar.BlockNumber,
			"index":       sidecar.Index,
			"root":        fmt.Sprintf("%#x", sidecar.BlockRoot()),
		}).Debug("Ignoring a duplicate blob sidecar save attempt")
		return nil
	}
...
func (bs *BlobStorage) Get(root [32]byte, idx uint64) (types.VerifiedROBlob, error) {
	startTime := time.Now()
	...
	return VerifiedROBlobFromDisk(bs.fs, root, bs.layout.sszPath(ident))
}
...
func VerifiedROBlobFromDisk(fs afero.Fs, root [32]byte, path string) (types.VerifiedROBlob, error) {
