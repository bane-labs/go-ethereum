...
func (pool *LegacyPool) Filter(tx *types.Transaction) bool {
	...
	if res && pool.config.AMEVCache {
		// Check if the transaction is an envelope transaction, or a DKG transaction
...
func (pool *LegacyPool) Init(gasTip uint64, head *types.Header, reserver txpool.Reserver) error {
	...
	pool.InitStatic(gasTip, head, nil, reserver, true)
...
func (pool *LegacyPool) InitStatic(gasTip uint64, head *types.Header, statedb *state.StateDB, reserver txpool.Reserver, allowMissingState bool) error {
		...
		case <-reannounce.C:
			...
			reannoTxs := func() []*types.Transaction {
				...
				for _, list := range pool.pending {
					...
					if !pool.config.ReannounceRemotes {
						continue
					}
