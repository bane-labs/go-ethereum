...
func (c *DBFT) processBlockCb(b dbft.Block[common.Hash]) error {
	...
	if c.chain.Config().IsNeoXAMEV(dbftBlock.header.Number) || c.chain.Config().IsNeoXEthSig(new(big.Int).Add(dbftBlock.header.Number, bigOne)) {
		c.lock.RLock()
		ks := c.amevKeystore
		c.lock.RUnlock()
...
func (c *DBFT) newBlockFromContextCb(ctx *dbft.Context[common.Hash]) dbft.Block[common.Hash] {
	if !c.chain.Config().IsNeoXAMEV(big.NewInt(int64(ctx.BlockIndex))) &&
		!c.chain.Config().IsNeoXEthSig(big.NewInt(int64(ctx.BlockIndex))) {
	...
	if !c.chain.Config().IsNeoXEthSig(big.NewInt(int64(ctx.BlockIndex + 1))) {
...
func (c *DBFT) newPrepareRequestCb(ts uint64, nonce uint64, txHashes []common.Hash) dbft.PrepareRequest[common.Hash] {
	...
	if c.chain.Config().IsNeoXEthSig(new(big.Int).Add(header.Number, bigOne)) {
		header.MixDigest = threshold
	} else if c.chain.Config().IsNeoXAMEV(new(big.Int).Add(header.Number, bigOne)) {
		header.MixDigest = threshold
	} else {
	...
	if c.chain.Config().IsNeoXAMEV(new(big.Int).Add(header.Number, bigOne)) &&
		!c.chain.Config().IsNeoXEthSig(new(big.Int).Add(header.Number, bigOne)) {
...
func (c *DBFT) verifyPrepareRequestCb(p dbft.ConsensusPayload[common.Hash]) error {
	...
	if req.SealingProposal.ParentHash != c.lastBlockHash {
		...
		if c.chain.Config().IsNeoXEthSig(new(big.Int).Sub(req.SealingProposal.Number, bigOne)) {
			return fmt.Errorf("invalid parent hash after NeoXAMEV with threshold signing scheme: expected %s, got %s", c.lastBlockHash, req.SealingProposal.ParentHash)
		}
...
func (c *DBFT) verifyBlockCb(b dbft.Block[common.Hash]) bool {
	if !c.chain.Config().IsNeoXAMEV(big.NewInt(int64(c.dbft.Context.BlockIndex))) &&
		!c.chain.Config().IsNeoXEthSig(big.NewInt(int64(c.dbft.Context.BlockIndex))) {
		dbftBlock := b.(*Block)
		parent := c.chain.CurrentBlock()
		if parent.Number.Cmp(dbftBlock.header.Number) >= 0 {
		...
		if c.chain.Config().IsNeoXEthSig(new(big.Int).Add(dbftBlock.header.Number, bigOne)) {
			expected = threshold
		} else if c.chain.Config().IsNeoXAMEV(new(big.Int).Add(dbftBlock.header.Number, bigOne)) {
			expected = threshold
		} else {
			expected = multisig
		}
...
func (c *DBFT) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header, isSealed bool) error {
	...
	if isV2Extra {
		expectedHashableExtraLen = dbftutil.HashableExtraV2Len
	} else if isV1Extra {
	...
	if isSealed {
		...
		if isV2Extra {
			expected = dbftutil.HashableExtraV2Len + tpke.PublicKeyLen + tpke.SignatureLen
		} else if isV1Extra {
...
func (c *DBFT) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	if chain.Config().IsNeoXEthSig(new(big.Int).Add(header.Number, bigOne)) {
		header.Extra = []byte{byte(dbftutil.ExtraV2)}
	} else if chain.Config().IsNeoXAMEV(new(big.Int).Add(header.Number, bigOne)) {
...
func (c *DBFT) waitForNewSealingProposal(desiredHeight uint64, updateContext bool) error {
	...
	if b.ParentHash().Cmp(c.lastBlockHash) != 0 {
		...
		if c.chain.Config().IsNeoXEthSig(new(big.Int).Sub(b.Number(), bigOne)) {
			return fmt.Errorf("invalid sealing task: parent hash mismatch with NeoXEthSig fork enabled: expected %s, got %s", c.lastBlockHash, b.ParentHash())
		}
...
func honestSealHash(header *types.Header) ([]byte, error) {
	...
	case dbftutil.ExtraV2:
		b := HonestSealHashV2(header).Bytes()
		sealHash = b[:]
	default:
...
// HonestSealHashV2 returns the hash of a block prior to it being sealed. It differs
// from WorkerSealHash in that all block fields except Extra's signature bytes are being
// hashed. This hash represents a G2 point on BLS12381 curve.
func HonestSealHashV2(header *types.Header) *bls12381.G2Affine {
	return HonestSealHashV1(header)
}
...
func encodeSigHeader(w io.Writer, header *types.Header) {
	...
	case dbftutil.ExtraV2:
		hashableExtraLen = dbftutil.HashableExtraV2Len
	default:
...
func (c *DBFT) getNextConsensus(h *types.Header, s *state.StateDB) (common.Hash, common.Hash) {
	...
	if !c.chain.Config().IsNeoXAMEV(new(big.Int).Add(h.Number, bigOne)) &&
		!c.chain.Config().IsNeoXEthSig(new(big.Int).Add(h.Number, bigOne)) {
