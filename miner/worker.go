...
func (miner *Miner) prepareWork(ctx context.Context, genParams *generateParams, witness bool) (result *environment, err error) {
	...
	if miner.chainConfig.IsLondon(header.Number) {
		...
		header.BaseFee = eip1559.CalcBaseFeeDBFT(miner.chainConfig, parent, state)
	...
	if err != nil {
		log.Error("Failed to pre-execute system calls", "err", err)
		return nil, err
	}
	env.bal.Merge(bal)
	return env, nil
...
func (miner *Miner) commitTransaction(ctx context.Context, env *environment, tx *types.Transaction) (err error) {
	...
	isEnvelope := antimev.IsEnvelope(tx)
	if miner.chainConfig.IsNeoXAMEV(env.header.Number) && isEnvelope {
		...
		if new(big.Int).SetUint64(uint64(env.ecount)).Cmp(envelopNumberLimit) >= 0 {
			log.Trace("Ignoring envelope transaction more than limit", "hash", tx.Hash, "limit", envelopNumberLimit)
			return nil
		}
	}
