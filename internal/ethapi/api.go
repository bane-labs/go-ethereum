...
func flattenTxs(txs types.Transactions, header *types.Header, cfg *params.ChainConfig, baseFee *big.Int) map[string]*RPCTransaction {
	dump := make(map[string]*RPCTransaction, len(txs))
	for _, tx := range txs {
		dump[fmt.Sprintf("%d", tx.Nonce())] = NewRPCPendingTransaction(tx, header, cfg, baseFee)
	}
	return dump
}
...
func (api *TxPoolAPI) Content() map[string]map[string]map[string]*RPCTransaction {
	...
	baseFee := api.nextBaseFeeCache.getNextBaseFee(curHeader.Number.Int64())
	// Flatten the pending transactions
	for account, txs := range pending {
		content["pending"][account.Hex()] = flattenTxs(txs, curHeader, api.b.ChainConfig(), baseFee)
	}
	// Flatten the queued transactions
	for account, txs := range queue {
		content["queued"][account.Hex()] = flattenTxs(txs, curHeader, api.b.ChainConfig(), baseFee)
	}
	return content
}
...
func (api *TxPoolAPI) ContentFrom(addr common.Address) map[string]map[string]*RPCTransaction {
	...
	content["pending"] = flattenTxs(pending, curHeader, api.b.ChainConfig(), baseFee)

	// Build the queued transactions
	content["queued"] = flattenTxs(queue, curHeader, api.b.ChainConfig(), baseFee)
...
func (api *BlockChainAPI) GetBlobSidecarByTxHash(ctx context.Context, hash common.Hash, fullBlob *bool) (map[string]interface{}, error) {
	...
	found, tx, blockHash, _, _ := api.b.GetCanonicalTransaction(hash)
...
// DoCallAtState executes the given transaction on the specified state with the given header. It's a
// handler for CallAtState and differs from the latter in that it returns the call result as is.
func DoCallAtState(ctx context.Context, b Backend, args TransactionArgs, state *state.StateDB, header *types.Header, overrides *override.StateOverride, blockOverrides *override.BlockOverrides, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	defer func(start time.Time) { log.Debug("Executing EVM call finished", "runtime", time.Since(start)) }(time.Now())
...
func (api *BlockChainAPI) EstimateGas(ctx context.Context, args TransactionArgs, blockNrOrHash *rpc.BlockNumberOrHash, overrides *override.StateOverride, blockOverrides *override.BlockOverrides) (hexutil.Uint64, error) {
	if err := args.setFeeMinAllowed(ctx, api.b); err != nil {
		return 0, err
	}
...
func (api *TransactionAPI) GetTransactionByHash(ctx context.Context, hash common.Hash) (*RPCTransaction, error) {
	...
	if !found {
		...
		if tx := api.b.GetPoolTransaction(hash); tx != nil {
			baseFee := api.nextBaseFeeCache.getNextBaseFee(api.b.CurrentHeader().Number.Int64())
			return NewRPCPendingTransaction(tx, api.b.CurrentHeader(), api.b.ChainConfig(), baseFee), nil
		}
...
func (api *TransactionAPI) PendingTransactions() ([]*RPCTransaction, error) {
	...
	for _, tx := range pending {
		...
		if _, exists := accounts[from]; exists {
			transactions = append(transactions, NewRPCPendingTransaction(tx, curHeader, api.b.ChainConfig(), baseFee))
		}
	}
