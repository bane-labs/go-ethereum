...
func (d *Downloader) syncToHead() (err error) {
	...
	// Switch to full sync if the network chain is too short for snap sync.
	if mode == ethconfig.SnapSync && latest.Number.Uint64() < uint64(fsMinFullBlocks*2) {
		log.Debug("Network chain too short for snap sync, switching to full sync", "height", latest.Number.Uint64())
